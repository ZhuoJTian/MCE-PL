...
from models.layers_att3 import SubnetConvUnstructured,SubnetLinear
...
def freeze_vars(model, var_name, freeze_bn=False):
    ...
    """
    assert var_name in ["weight", "bias", "popup_scores_local", "momen_value"]  
    for i, v in model.named_modules():
        if hasattr(v, var_name):
            if not isinstance(v, (nn.BatchNorm2d, nn.BatchNorm2d, nn.Conv1d)) or freeze_bn:
...
def initialize_scores(model, init_type):
    ...
    for m in model.modules():
        if hasattr(m, "popup_scores_local"):
            if init_type == "kaiming_uniform":
                nn.init.kaiming_uniform_(m.popup_scores_local)
            elif init_type == "kaiming_normal":
                nn.init.kaiming_normal_(m.popup_scores_local)
            elif init_type == "xavier_uniform":
                nn.init.xavier_uniform_(
                    m.popup_scores_local, gain=nn.init.calculate_gain("relu")
                )
            elif init_type == "xavier_normal":
                nn.init.xavier_normal_(
                    m.popup_scores_local, gain=nn.init.calculate_gain("relu")
                )
        elif hasattr(m, "popup_scores"):
            if init_type == "kaiming_uniform":
...
def initialize_weight(model, init_type):
    # print(f"Initialization relevance score with {init_type} initialization")
    for m in model.modules():
        if hasattr(m, "weight"):
            if init_type == "kaiming_uniform":
...
def switch_to_prune(model):
    ...
    unfreeze_vars(model, "popup_scores_local")
    unfreeze_vars(model, "momen_value")
...
def switch_to_finetune(model):
    ...
    # )
    freeze_vars(model, "popup_scores_local")
...
def prepare_model(model, args):
    ...
    initialize_weight(model, args.scores_init_type)
