...
## to get the data volumn of the model
def getModelSize_Weig(weight):
    param_size = 0
    for w in weight:
        param_size += torch.count_nonzero(w) * 32
        # print("a")
    return param_size.item() *1.25/(10**7)

def getModelSize_Mask(mask):
    param_size = 0
    for m in mask:
        param_size += torch.sum(m)
        # print("a")
    return param_size.item()*1.25/(10**7)
