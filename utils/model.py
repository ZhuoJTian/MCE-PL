...
from models.layers import SubnetConvUnstructured, SubnetLinear
...
def freeze_vars(model, var_name, freeze_bn=False):
    ...
    assert var_name in ["weight", "bias", "popup_scores_local"]  
...
def prepare_model(model, args):
    ...
    if args.exp_mode == "pretrain":
        ...
        freeze_vars(model, "popup_scores_local", args.freeze_bn)
        unfreeze_vars(model, "weight")
        unfreeze_vars(model, "bias")
