...
def create_datasets(data_path, dataset_name, num_clients, local_dist, local_dist_test):
    ...
    for i in range(num_clients):
        ...
        for cls in range(10):
            if local_dist[i][cls] > 0:
                for num in range(int(remained[cls]), int(remained[cls] + int(local_dist[i][cls]/5))):
                    index.append(int(sample_index_train[cls][num]))
                remained[cls] += int(local_dist[i][cls]/5)
    ...
    split_datasets_train = [((torch.Tensor(training_dataset.data)[torch.tensor(idx_clients_train[i])]),
                            (torch.Tensor(training_dataset.targets)[torch.tensor(idx_clients_train[i])]).long())
                            for i in range(num_clients)]
    
    split_datasets_val = [((torch.Tensor(training_dataset.data)[torch.flip(torch.tensor(idx_clients_train[i]), dims=[0])]),
                            (torch.Tensor(training_dataset.targets)[torch.flip(torch.tensor(idx_clients_train[i]), dims=[0])]).long())
                            for i in range(num_clients)]


    split_datasets_test = [((torch.Tensor(test_dataset.data)[torch.tensor(idx_clients_test[i])]),
                            (torch.Tensor(test_dataset.targets)[torch.tensor(idx_clients_test[i])]).long())
                            for i in range(num_clients)]

    local_datasets_train = [
        CustomTensorDataset(local_dataset, transform=transform)
            for local_dataset in split_datasets_train
    ]

    local_datasets_val = [
        CustomTensorDataset(local_dataset, transform=transform)
            for local_dataset in split_datasets_val
    ]

    local_datasets_test = [
        CustomTensorDataset(local_dataset, transform=transform)
        for local_dataset in split_datasets_test
    ]

    return local_datasets_train, local_datasets_val, local_datasets_test
